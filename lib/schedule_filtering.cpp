...
bool ScheduleLikabilityCalculator::compareStartTimes(courseTimes x, courseTimes y){return x.start < y.start;}
double ScheduleLikabilityCalculator::normalizeDaysOff(double min, double max, double num){
if (max == min) return 1.0;
return ((num - min)/(max - min));
}
double ScheduleLikabilityCalculator::normalizeClassesPerDay(double min, double max, double num){
if (max == min) return 1.0;
return (1 - (num - min)/(max - min));
}
double ScheduleLikabilityCalculator::normalizeGapHours(double min, double max, double num){
if (max == min) return 1.0;
return (1 - (num - min)/(max - min));
}
void ScheduleLikabilityCalculator::scorePermutations(){
    nonScoredSchedules = coursesData.conflictResolver();
    if (nonScoredSchedules.empty()) {return;}
    const auto& courses = coursesData.getCourses();
...
void ScheduleLikabilityCalculator::normalizeValues()
{
...
void ScheduleLikabilityCalculator::calculateFinalSchedules()
{
...
void ScheduleLikabilityCalculator::schedulesToObjects(){
...
const vector <Schedule>& ScheduleLikabilityCalculator::getFinalSchedules(){
